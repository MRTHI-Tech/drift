...
- **runs/{runId}** — `projectId`, `trigger` (`scheduled` | `deploy` | `manual`), `startedAt`, `finishedAt`, `routesChecked`, `status` (`clean` | `findings` | `error`), `findingIds` (array), `knownFindings` (nullable number: candidates this run found that an existing finding already covered, so a `clean` run can say whether it found nothing or only found what is already known; null on runs written before the field existed, which is not the same as zero), `error` (nullable).
...
- **conventions/{conventionId}** — `projectId`, `archetypeId` (nullable for product-wide), `property` (e.g. `cta.label`, `heading.size`, `copy.case`, or a component property: a kind and a CSS property joined by a dot, `button.border-radius`, `icon.color`), `value`, `label` (model-written, user-editable, the convention in plain language), `confidence` (`low` | `medium` | `high`), `evidenceScreenIds`, `exceptions` (array of `{screenId, reason}`), `status` (`derived` | `promoted` | `removed`), `updatedAt`.
- **findings/{findingId}** — `projectId`, `runId`, `type` (`token` | `pattern`), `screenId`, `conventionId` (nullable for token findings), `evidence` (structured: selector (nullable, the element the value was seen on), property, observedValue, expectedValue, expectedSource (nullable, the token or convention the expected value comes from), siblingScreenIds, sentence (nullable, the one-line reading of the evidence in plain language; written by the judgment phase for a pattern finding after its value passed the reconciliation gate, and composed by Drift for a component finding, which no model proposed and which therefore has nothing for the gate to verify)), `severity`, `status` (`open` | `resolved_conform` | `resolved_update_siblings` | `resolved_exception` | `dismissed`), `dedupeKey`, `verifiedAt` (nullable: when a run rendered the product and found the value gone, after somebody resolved the finding by conforming the screen; absent on a finding written before the field existed, and null forever on one nobody claimed to have fixed), `prNumber` (nullable), `createdAt`, `resolvedAt`.
...
- A resolution is a decision and `verifiedAt` is the evidence the decision took effect. They are different facts, and a finding carrying only the first is a finding nobody has checked. A run re-renders the product and asks the question the pull request could not answer: a claimed fix whose value is still being rendered is reopened, and one whose value is gone is stamped. Only `resolved_conform` is asked about, because that action means this screen should change; `resolved_update_siblings` means the opposite and an exception means a person already decided.
...
- Conventions require **3 or more agreeing** before they exist at all. Two agreeing is not a convention. What is counted depends on what the convention is about, and there are exactly two units. A convention over a **screen** property counts screens, and the floor is enforced in the repository. A convention over a **component** property counts instances of that component, because the unit there is the control rather than the page it sits on: four buttons agreeing across two screens is real evidence about buttons, and a floor written in screens would refuse it. That floor is enforced where the counting happens, in `deriveComponentConventions`, which is the only place the instance count exists; the repository still refuses a convention nothing at all was measured on.
- A component convention is **product-wide by construction** and consults no archetype at any point. Two screens that resemble each other are already compared, two that do not are never compared, and a radio does not care which screen it is on. Only a **high-confidence** component convention raises a finding. A weaker one is still stated, because the rules file is worth telling somebody that most of their buttons are pills, and only a standard is worth interrupting them over.
