...
describe("computeDriftScore", () => {
  ...
  it("scores one open finding per screen checked at 0", () => {
    expect(computeDriftScore({ openFindings: 8, screensChecked: 8 })).toBe(0)
  })

  it("reads as the share of screens that are clean", () => {
    expect(computeDriftScore({ openFindings: 2, screensChecked: 8 })).toBe(75)
    expect(computeDriftScore({ openFindings: 1, screensChecked: 3 })).toBe(67)
  })

  it("floors at 0 when there are more findings than screens", () => {
    expect(computeDriftScore({ openFindings: 40, screensChecked: 8 })).toBe(0)
  })
...
describe("driftScoreSeries", () => {
  ...
  it("reconstructs the score as it stood at the end of each run, oldest first", () => {
    ...
    expect(points.map((point) => point.openFindings)).toEqual([1, 2, 1])
    expect(points.map((point) => point.score)).toEqual([75, 50, 75])
  })
...
describe("refreshDriftScore", () => {
  ...
  it("stores the score and reports that it moved", async () => {
    ...
    expect(refreshed).toMatchObject({ openFindings: 1, screensChecked: 2, score: 50, changed: true })
    expect(repositories.stored.projects[0]?.driftScore).toBe(50)
  })

  it("writes nothing when the project already carries this score", async () => {
    const repositories = fakeRepositories({
      projects: [{ ...PROJECT, driftScore: 50 }],
      screens: twoScreens,
      findings: [PATTERN_FINDING],
    })
